class Product < ApplicationRecord
  ...
  has_many :discount_sliders, dependent: :destroy
  ...
  def final_price
    ...
    number_with_precision(price - ((price * discount_percentage)/100), precision: 2)
  end

  def before_import_save(record)    
    self.remote_product_image_url = record[:product_image] if record[:product_image].present?
  end
