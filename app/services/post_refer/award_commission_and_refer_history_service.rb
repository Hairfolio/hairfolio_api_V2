  ...
  class AwardCommissionAndReferHistoryService
    ...
    def reward_wallet_commission(refer)
      if refer.post.present?
        ...
        post_creator.wallet.update(amount: wallet_amount +  wallet_commission_list.commission_amount) if wallet_commission_list.present?
      else
        post_creator = refer.user
        wallet_commission_list =
            WalletCommissionList.create(order_detail_id: order_detail.id, user_id: post_creator&.id,
                                        commission_amount: (order_detail.price * WalletCommissionList.log_commission_percentage)/100) if post_creator.id != cart.user_id
        wallet_amount = post_creator.wallet&.amount || 0
        post_creator.wallet.update(amount: wallet_amount +  wallet_commission_list.commission_amount) if wallet_commission_list.present?
      end
    end
