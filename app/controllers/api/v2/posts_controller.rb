class Api::V2::PostsController < ApplicationController
  ...
  def post_params
    params.require(:post).permit(:description,products_attributes: [:id, :_destroy], product_ids:[], videos_attributes: [:id, :_destroy, :asset_url, :post_id, :video_url], photos_attributes: [:asset_url, :video_url,:thumb_url, :id, :_destroy, labels_attributes: [:id, :_destroy, :post_id, :product_id, :tag_id, :label_type, :position_top, :position_left, :name, :url, formulas_attributes: [:id, :_destroy, :service_id, :line_id, :time, :weight, :volume, :position_top, :position_left, :post_id, treatments_attributes: [:color_id, :weight, :id, :_destroy]]]])
  end
