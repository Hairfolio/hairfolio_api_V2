class Api::V1::ProductsController < ApplicationController
  ...
  before_action :authenticate_with_token!, only: [:all_products,:csv_import]
  ...
  # end
  def all_products
    products = Product.select(:id, :name,:product_image)
    products = Product.where('name ilike ?', "%#{params[:q]}%") if params[:q].present?
    render json: products, each_serializer: ProductMinimalSerializer
  end

  def csv_import
    if params[:file].present?
      resp = Products::ProductImport.new(file: params[:file]).apply 
      if resp == "success"
        render json: {message: "import successfully"}
      else
        render json: {message: "column not match"}
      end
    else
      render json: {message: "File Not Upload"}
    end
  end
