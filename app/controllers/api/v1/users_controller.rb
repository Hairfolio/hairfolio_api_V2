class Api::V1::UsersController < ApplicationController
  ...
  before_action :authenticate_with_token!, only: %i[index fetch_wallet destroy change_password update]
  ...
  def userposts
    ...
    render json: @posts, user_id: @user_id, meta:pagination_dict(@posts),each_serializer: OwnPostSerializer
  end

  def get_user_profile
    user = User.find(params[:id])
    render json: user, user_id: @user_id, serializer: UserProfileSerializer
  end
  ...
  def stylist_near_me
    if request.headers['Authorization'].present? && params[:latitude].present? && params[:longitude].present?
  ...
  def user_likes
    ...
    likes = likes.page(params[:page]).per(params[:limit])

    render json: likes, user_id: @user_id, meta: pagination_dict(likes), status: 201
  end
  ...
  def destroy
    if @user.destroy
      ...
      render json: { error: "The system won't allow this action , because other objects are depend on it." }, status: 422
    end
  end
  ...
  def fetch_wallet
    commission_amount = current_user.wallet_commission_lists.pluck(:commission_amount).sum
    render json: { wallet: { id: current_user.wallet.id,amount: "%.2f" % current_user.wallet.amount,user_id: current_user.wallet.user_id}, commission_amount: {amount: "%.2f" % commission_amount} }
  ...
  # def user_params
  #   params.require(:user).permit(:device_id, :facebook_token, :twitter_token, :instagram_token, :default_pinterest_board, :pinterest_token, :email, :password, :password_confirmation, :account_type, :first_name, :last_name, :description, :avatar_url, :avatar_cloudinary_id, :share_facebook, :share_twitter, :share_instagram, :share_pinterest, :share_tumblr, :prof_desc, :years_exp, :salon_id, :career_opportunity, brand_attributes: [:id, :_destroy, :name, :info, :address, :city, :state, :zip, :website, :phone], salon_attributes: [:id, :name, :info, :address, :city, :state, :zip, :website, :phone], educations_attributes: [:name, :year_from, :year_to, :degree_id, :user_id, :website, :id, :_destroy], offerings_attributes: [:user_id, :category_id, :service_id, :price, :id, :_destroy], certificate_ids: [], experience_ids: [])
  # end
  def user_params
    params.require(:user).permit(:device_id, :facebook_token, :twitter_token, :instagram_token, :default_pinterest_board, :pinterest_token, :email, :password, :password_confirmation, :account_type, :first_name, :last_name, :description, :avatar_url, :avatar_cloudinary_id, :share_facebook, :share_twitter, :share_instagram, :share_pinterest, :share_tumblr, :prof_desc, :years_exp, :salon_id, :career_opportunity, brand_attributes: [:id, :_destroy, :name, :info, :address, :city, :state, :zip, :website, :phone], salon_attributes: [:id, :name, :info, :address, :city, :state, :zip, :website, :phone], workplace_attributes: [:id, :name, :info, :address, :city, :state, :zip, :website, :phone,:salon_id], educations_attributes: [:name, :year_from, :year_to, :degree_id, :user_id, :website, :id, :_destroy], offerings_attributes: [:user_id, :category_id, :service_id, :price, :id, :_destroy], certificate_ids: [], experience_ids: [])
  end
