RailsAdmin.config do |config|
  ...
  config.model Product do
    edit do
      ...
      exclude_fields :favourites,:carts, :posts, :tag, :favourites_count, :relations, :sale, :item, :active_sale_item, :active_sale,:categories,:discount_sliders,:store_shop_its,:order_details
    end
    list do
      field :id
      field :name
  ...
  config.model GlobalVar do
    ...
    edit do
      ...
      field :value, :integer do
        help 'Required - Enter percentage value'
      end
    end
  end

  config.model LogDefault do
    label "Stylist's default Title and Description"
    list do
      field :id
      field :title
      field :description
  ...
  config.model MobileStoreBanner do
    edit do
      field :name do
        visible false
      end
      field :banner_image do
        label "Banner Image (679*679px)"
      end
      include_all_fields
    end
  end
  config.model Workplace do
    visible false
  end
  ...
  config.actions do
    ...
    new do
             ...
             'SalonImage', 'SalonTiming', 'SalonBusinessInfo', 'Section', 'SalonService', 'Post','Coupon','ReferHistory','MobileStoreBanner'].append(Sale.add_for_new)
    end
    export do
              ...
              'SalonImage', 'SalonTiming', 'SalonBusinessInfo', 'Section', 'SalonService','Homepage','DiscountSlider','MobileStoreBanner']
    end
    bulk_delete do
      except ['Relation', 
              ...
              'SalonImage', 'SalonTiming', 'SalonBusinessInfo', 'Section', 'SalonService', 'DiscountSlider', 'Post','MobileStoreBanner']
    end
    show do
    ...
    delete do
      except ['PushNotification', 'Order', 'ReferralHistory', 'GlobalVar','LogDefault', 'Relation',
               # 'ProductType', 'Preference', 'ConsistencyType', 'StylingTool', 'Shampoo', 'Conditioner',
              'Collection', 'StylingProduct', 'Sale', 'Item',
              'SalonImage', 'SalonTiming', 'SalonBusinessInfo', 'Section', 'SalonService','Homepage','DiscountSlider','Coupon','ReferHistory', 'Refer','MobileStoreBanner']
    end
    show_in_app do
      except ['Cart', 'Relation', 'Item']
    end
    import do
      except ['ReferralHistory', 'GlobalVar','LogDefault','Relation',
              ...
              'SalonImage', 'SalonTiming', 'SalonBusinessInfo', 'Section', 'SalonService','Homepage','DiscountSlider', 'Post','MobileStoreBanner']
    end
    nestable
